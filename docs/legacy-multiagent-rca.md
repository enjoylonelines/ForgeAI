# ForgeAI — 멀티에이전트 RAG·RCA 구현 이력 (레거시)

> 이 문서는 프로젝트 초기 범위였던 **FDC·RCA 멀티에이전트 시스템**의 설계·구현 기록입니다.
> 현재 포트폴리오의 주 구현 범위는 하이브리드 정비 정책이며, 그 내용은 [README](../README.md)에 있습니다.
> 아래 내용은 설계 판단의 이력으로 보존합니다.
