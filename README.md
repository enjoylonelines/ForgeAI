...
→ 이 빈칸은 SECOM 또는 AI4I recall/precision 트레이드오프 곡선을 그려  
"Recall ≥ X에서 Precision이 얼마나 떨어지는가"를 측정한 후 채운다.  
현재 XGBoost 기본 임계값(0.5) 기준 Recall 0.779 / Precision 0.747.
...
```
배포 승인 조건 (AND 전부 충족):
  PR-AUC         ≥ [___]    ← 운영점 결정 후 기준선 확정 (현재 XGBoost 0.830 참고)
  운영점 Recall  ≥ [___]    ← "Recall ≥ X에서 Precision이 얼마인가" 곡선 측정 후
